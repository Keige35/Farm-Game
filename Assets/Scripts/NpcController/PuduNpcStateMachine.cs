...
public class PuduNpcStateMachine : Damageable
{
    ...
    public List<float> DamagePercent => damagePercent;
    ...
    protected override void HealthUpdated()
    {
        ...
        currentPercent = (float)currentHealth / maxHealth;
        currentPercent *= 100f;
        UpdatePercent(currentPercent);

        itemSpawner ??= ServiceLocator.GetService<ItemSpawner>();
        itemSpawner.SpawnEffect(EffectType.Hit, transform.position);
        UpdateRunState();
        if (currentHealth > 0)
        {
            return;
    ...
    public void SetDamnagePercent(List<float> percentList)
    {
        damagePercent.Clear();
        foreach (float percent in percentList)
        {
